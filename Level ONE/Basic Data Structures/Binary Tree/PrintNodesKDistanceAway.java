...
public class PrintNodesKDistanceAway {
    public static class Node {
      ...
      public static void display(Node node) {
        ...
        public static void printKLevelsDown(Node node, int k, Node avoid){
            if(node == avoid || node == null)
                return;
            if(k==0){
                System.out.println(node.data);
                return;
            }
            printKLevelsDown(node.left, k-1, avoid);
            printKLevelsDown(node.right, k-1, avoid);
        }
        ...
        public static void printKNodesFar(Node node, int data, int k) {
            ...
            for(int i=0; i<ntrp.size(); i++){
                printKLevelsDown(ntrp.get(i), k-i, avoid);
                avoid = ntrp.get(i);
            }
        }
