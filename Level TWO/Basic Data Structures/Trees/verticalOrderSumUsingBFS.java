...
public class verticalOrderSumUsingBFS {
    ...
    public static ArrayList<Integer> verticalOrderSum(TreeNode root) {
        ...
        while(q.size()>0){
            Pair curr = q.remove();
            ans.set(curr.hl, ans.get(curr.hl)+curr.node.val);
            
            if(curr.node.left!=null) q.add(new Pair(curr.node.left, curr.hl-1));
            if(curr.node.right!=null) q.add(new Pair(curr.node.right, curr.hl+1));
        }
        return ans;
    }
